...
export const approveEmployer = async (req, res) => {
    try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 30px; border: 1px solid #e0e0e0; border-radius: 10px; background-color: #f9f9f9;">
          ...
          <p style="font-size: 16px; color: #444;">Dear ${employer.companyName || "Employer"},</p>
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="https://earnease.vercel.app/login" style="background-color: #4CAF50; color: #fff; padding: 12px 25px; text-decoration: none; border-radius: 5px; font-size: 16px;">
              Log In Now
            </a>
          </div>
      ...
      `,
        ...
        res.status(200).json({ success: true, message: "Employer approved successfully" })
    } catch (err) {
...
export const rejectEmployer = async (req, res) => {
    try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 30px; border: 1px solid #e0e0e0; border-radius: 10px; background-color: #fff3f3;">
          ...
          ${rejectionReason
                    ? `<p style="font-size: 16px; color: #444;"><strong>Reason:</strong> ${rejectionReason}</p>`
                    : ""
                }
          <div style="text-align: center; margin: 30px 0;">
            <a href="https://earnease.vercel.app/register/employer" style="background-color: #d32f2f; color: #fff; padding: 12px 25px; text-decoration: none; border-radius: 5px; font-size: 16px;">
              Reapply Now
            </a>
          </div>
          <p style="font-size: 14px; color: #999;">
